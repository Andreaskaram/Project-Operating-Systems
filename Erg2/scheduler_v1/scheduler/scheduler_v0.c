...
int main(int argc,char **argv)
{
	...
	while ((c = fscanf(input, "%s", exec)) != EOF) {
    	...
    	if (fscanf(input, "%d", &numCores) == 1) {
        	proc->reqCores = numCores; // Assign the number if it exists
    	} else {
        	proc->reqCores = 1; // Default to 0 if the number doesn't exist
    	}
